...
function parseLignes(raw) {
  if (!raw) return { mo: [], mat: [] };
  if (typeof raw === "string") { try { return JSON.parse(raw); } catch { return { mo: [], mat: [] }; } }
  return raw;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      const rows = [
        ...
        ...(lignes.mo || []).map(l => {
          const sv = SERVICES_MO.find(x => x.id === l.service);
          return { t: `  • ${sv?.label || l.service}${l.quantite ? ` : ${l.quantite} ${sv?.unite?.toUpperCase() || "H"}` : ""}`, s: "item" };
        }),
        ...(lignes.mat?.length > 0 ? [
          { t: "", s: "sp" },
          { t: "MATÉRIEL", s: "sec" },
          ...(lignes.mat || []).map(l => ({
            t: `  • ${l.designation}${l.eldas ? ` | ELDAS: ${l.eldas}` : ""}${l.fournisseur ? ` | ${l.fournisseur === "Autre" ? l.fournisseur_custom || "Autre" : l.fournisseur}` : ""}${l.quantite ? ` | Qté: ${l.quantite}` : ""}`,
            s: "item"
          })),
        ] : []),
        { t: "", s: "sp" },
      ...
      ctx.fillStyle = "#F59E0B"; ctx.font = "bold 18px Arial";
      ctx.fillText("⚡ " + APP_NAME, PAD, 50);
      ctx.fillStyle = "#9ca3af"; ctx.font = "12px Arial";
      ctx.fillText("Bon de régie — Plus values", W - 280, 50);
      ...
      ctx.strokeRect(PAD, sigAreaY, W - PAD * 2, sigH - 30);
...
function SignatureCanvas({ regie, chantier, onSave, onCancel }) {
  ...
  const stop  = (e) => {
    ...
    if (ref.current && !isCanvasEmpty(ref.current)) setIsEmpty(false);
  };
  const draw = (e) => {
  ...
  const clear = () => {
    ...
    setIsEmpty(true); setSigErr(false);
  };
  const validate = () => {
    if (isCanvasEmpty(ref.current)) { setSigErr(true); return; }
      ...
      <div style={{ ...S.modal, paddingBottom: 32 }}>
        ...
        <div style={{ background: "#0A0C12", borderRadius: 10, padding: 12, marginBottom: 12, maxHeight: 180, overflowY: "auto" }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#F59E0B", marginBottom: 4 }}>
            {chantier?.nom}{chantier?.numero ? ` — N° ${chantier.numero}` : ""}
          </div>
          <div style={{ fontSize: 11, color: "#9CA3AF", marginBottom: 4 }}>
            Régie #{regie?.regie_number} · {regie?.categorie} · {regie?.initiales} · {formatDate(regie?.created_at)}
          </div>
          ...
          {(lignes.mo || []).map((l, i) => {
            const sv = SERVICES_MO.find(x => x.id === l.service);
            return <div key={i} style={{ fontSize: 11, color: "#9CA3AF" }}>👷 {sv?.label || l.service}{l.quantite ? ` : ${l.quantite} ${sv?.unite?.toUpperCase() || "H"}` : ""}</div>;
          })}
          {(lignes.mat || []).filter(l => l.designation).map((l, i) =>
            <div key={i} style={{ fontSize: 11, color: "#9CA3AF" }}>📦 {l.designation}{l.quantite ? ` · Qté: ${l.quantite}` : ""}</div>
          )}
        </div>
...
export default function App() {
  ...
  const [saved,         setSaved]         = useState(null); // null | "success" | "fallback" | "error"
  ...
  // ── Signature handler with fallback ─────────────────────────────────────────
  const handleSignature = async (regieId, sigDataUrl) => {
    ...
    try {
      ...
      console.warn("Composite failed, using fallback:", e.message);
      mode = "fallback";
  ...
  const generateRecap = (chantierId) => {
    ...
    const num   = ch?.numero ? ` (N° affaire: ${ch.numero})` : "";
    const text  = [
      `RÉCAPITULATIF PLUS VALUES / RÉGIE`,
      `${APP_NAME}`,
      `Chantier : ${nom}${num}`,
  ...
  const renderToast = () => {
    ...
    const configs = {
      success:  { style: S.toastGreen,  text: "✓ Signature enregistrée !" },
        ...
        <div style={S.body}>
          ...
          <div style={S.section}>
            ...
            <div style={{ display: "flex", alignItems: "center" }}>
              <input
                style={{ ...S.input, borderRadius: "10px 0 0 10px", flex: 1 }}
                placeholder="nom"
                value={newCh.techCustom}
                onChange={e => setNewCh(f => ({ ...f, techCustom: e.target.value }))}
              />
              <div style={{ background: "#1E2235", border: "1px solid #1E2235", borderLeft: "none", borderRadius: "0 10px 10px 0", padding: "12px 10px", fontSize: 12, color: "#4B5563", whiteSpace: "nowrap" }}>
                @bourquinelectricite.ch
              </div>
            </div>
                  ...
                  <div key={l.id} style={{ background: "#0A0C12", border: "1px solid #1E2235", borderRadius: 10, padding: 10, marginBottom: 8 }}>
                    ...
                    {l.fournisseur === "Autre" && (
                      <div style={{ marginTop: 8 }}>
                        <label style={S.labelOpt}>Nom fournisseur</label>
  ...
  if (view === "recap") return (
    <div style={S.app}>
      ...
      <div style={S.body}>
        <div style={{ ...S.card, background: "rgba(245,158,11,0.06)", borderColor: "rgba(245,158,11,0.2)", marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: "#F59E0B", marginTop: 4 }}>
            {recap?.items.length} régie{recap?.items.length !== 1 ? "s" : ""}
          </div>
        </div>
            ...
            <div key={r.id} style={{ ...S.card, marginBottom: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#E8EAF0" }}>
                  ...
                  {r.photo_url && (
                    <span style={{ ...S.photoBadge, marginLeft: 6 }} onClick={() => setFullscreenUrl(r.photo_url)}>📷</span>
                  )}
                </div>
                <span style={{ fontSize: 11, color: "#4B5563" }}>{formatDate(r.created_at)}</span>
              </div>
              <div style={{ fontSize: 12, color: "#9CA3AF", marginBottom: 4 }}>
                {r.categorie}{r.emplacement ? ` · 📍 ${r.emplacement}` : ""}
              </div>
              ...
              {r.photo_url && (
                <img
                  src={r.photo_url}
                  onClick={() => setFullscreenUrl(r.photo_url)}
                  style={{ width: "100%", borderRadius: 8, maxHeight: 160, objectFit: "cover", marginBottom: 8, cursor: "pointer" }}
                  alt="photo régie"
                />
              )}

              {(lignes.mo || []).map((l, i) => {
                const s = SERVICES_MO.find(x => x.id === l.service);
                return <div key={i} style={{ fontSize: 12, color: "#9CA3AF", padding: "2px 0" }}>👷 {s?.label || l.service}{l.quantite ? ` : ${l.quantite} ${s?.unite?.toUpperCase() || "H"}` : ""}</div>;
              })}
              {(lignes.mat || []).filter(l => l.designation).map((l, i) => (
                <div key={i} style={{ fontSize: 12, color: "#9CA3AF", padding: "2px 0" }}>📦 {l.designation}{l.quantite ? ` · ${l.quantite}` : ""}</div>
              ))}
              ...
              {r.signature_url ? (
                <div style={{ ...S.sigBox, background: "rgba(16,185,129,0.08)", borderColor: "rgba(16,185,129,0.3)", marginTop: 8 }}>
                  ...
                  <img
                    src={r.signature_url}
                    onClick={() => setFullscreenUrl(r.signature_url)}
                    style={{ width: "100%", borderRadius: 6, maxHeight: 100, objectFit: "contain", background: "#fff", cursor: "pointer" }}
                    alt="document signé"
                  />
                  <button style={S.btnResign} onClick={() => setShowSig(r.id)}>↩ Re-signer</button>
                </div>
              ) : (
                <button style={{ ...S.btnSecondary, width: "100%", marginTop: 8 }} onClick={() => setShowSig(r.id)}>
        ...
        <button style={{ ...S.btnPrimary, marginTop: 12, background: "#1E2235", color: "#9CA3AF", border: "none" }}
      ...
      {showSig && (() => {
        const r  = regies.find(x => x.id === showSig);
        const ch = getChantier(r?.chantier_id) || {};
        return <SignatureCanvas regie={r} chantier={ch} onSave={(d) => handleSignature(showSig, d)} onCancel={() => setShowSig(null)} />;
      })()}
    ...
    <div style={S.app}>
      ...
      <div style={S.header}>
        <div style={S.logoWrap}>
          ...
          <div>
            <div style={S.logoTitle}>{APP_NAME}</div>
          ...
          <div style={S.alertBan}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#EF4444" }}>
                {enAttenteCount} régie{enAttenteCount !== 1 ? "s" : ""} sans signature
              </div>
              <div style={{ fontSize: 11, color: "#6B7280", marginTop: 2 }}>Ouvrir le récapitulatif pour faire signer</div>
            </div>
          </div>
        )}
        ...
        {filteredChantiers.length === 0 ? (
          <div style={S.emptyState}>
            ...
            <div style={{ fontSize: 13 }}>
              {dashTab === "archives" ? "Les chantiers archivés apparaîtront ici" : "Appuie sur + pour saisir ta première régie"}
            </div>
          </div>
        ) : (
                ...
                {nf.map((r, ridx) => {
                  ...
                  return (
                    <div key={r.id} style={S.regieRow}>
                      ...
                      {(lignes.mo || []).slice(0, 2).map((l, i) => {
                        const sv = SERVICES_MO.find(x => x.id === l.service);
                        return <div key={i} style={{ fontSize: 11, color: "#9CA3AF" }}>👷 {sv?.label || l.service}{l.quantite ? ` · ${l.quantite} ${sv?.unite?.toUpperCase() || "H"}` : ""}</div>;
                      })}
                      {(lignes.mo || []).length > 2 && (
                        <div style={{ fontSize: 11, color: "#4B5563" }}>+ {lignes.mo.length - 2} service{lignes.mo.length - 2 > 1 ? "s" : ""}</div>
                      )}
                      {(lignes.mat || []).filter(l => l.designation).length > 0 && (
                        <div style={{ fontSize: 11, color: "#4B5563", marginTop: 2 }}>📦 {lignes.mat.length} ligne{lignes.mat.length > 1 ? "s" : ""} matériel</div>
                      )}
